...
export default function Consulting() {
  ...
  const [elementDetail ,setElementDeatil] = useState();
 ...
 const handleSetElementDetail = (data) => {
    setElementDeatil(data);
 }
  ...
  const handleAPIKoi = async () => {
    ...
    try {
      const res = await api.get(
        `v1/fish/getKoiElement/${value.elementID}`
      );
      const resPond = await api.get(
        `v1/pond/getByElement/${value.elementID}`
      );
      ...
      console.log(res , resPond , resPost);
  ...
  const handleGetElement =  async (data) =>{
    ...
    try {
      const result = await api.get(`v1/user/getElement?elementID=${data}`)
  ...
  useEffect(() => { 
    ...
    //  setvalue(user)
     // call api date
     const {elementID} = user
      handleGetElement(elementID);
        ...
        <div className="max-w-full h-full">
          <div className="flex justify-center">
            <Calculator
              ...
              setElementDetail={handleSetElementDetail}
            />
          </div>
            ...
            <div className="tag-result mt-12">
              {value &&
                (loading ? (
                  ...
                  <div className="w-full flex flex-col gap-5  items-center content-center ">
                    <div className="w-[40%]   flex  flex-row items-center justify-between  ">
                      <h1 className="text-xl lg:text-4xl md:text-3xl sm:text-xl font-bold ">
                        Your Element is{" "}
                      </h1>
                      
                        <Tag
                          className="text-xl sm:text-4xl lg:text-4xl font-bold outline outline-1 outline-slate-700 rounded-lg "
                    ...
                    <div className="w-[40%]  flex flex-row items-center justify-between">
                      ...
                      <Tag className="text-xl sm:text-4xl lg:text-4xl font-bold outline outline-1 outline-slate-700 rounded-lg  "
                      color={colorArray[value.elementID - 1]}
                      >
                        {value.direction}
                    ...
                    <Slider {...settings}>
                      {post.map((newsItem) => (
                        <div key={newsItem.id} className="card">
                          ...
                          <div className="content text-[10px] px-0 sm:text-xs">
                            <span className="price">
                              <a href={`/details/${newsItem._id}`}>Read more</a>
                            </span>
                            <ul>
                              <li className="">
